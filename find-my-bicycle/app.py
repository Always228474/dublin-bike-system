...
def get_stations():
    try:
        stations_data = api_client.get_bike_stations()
...
def get_station_history(number):
    try:
        ...
        with db_manager.get_engine('bikes').connect() as conn:
...
def get_weather():
    try:
        data = api_client.get_current_weather()
        if not data:
...
def get_hourly_weather():
    try:
        ...
        with db_manager.get_engine('future').connect() as conn:
            result = conn.execute(text("""
...
def get_future_weather():
    try:
        data = api_client.get_weather_forecast()
        if not data:
...
def predict_bike_availability():
    try:
        ...
        # 获取天气特征（优先使用真实API，失败时使用模拟数据）
        weather_features = api_client.get_simulated_weather_features()
        avg_temp = weather_features['avg_temp']
        temp_range = weather_features['temp_range']
        temp_std = weather_features['temp_std']
...
def login():
    if request.method == "POST":
        ...
        try:
            with db_manager.get_engine('user').connect() as conn:
...
@app.route('/stations')
def stations_page():
    return render_template("stations.html", google_maps_api_key=GOOGLE_MAPS_API_KEY)
